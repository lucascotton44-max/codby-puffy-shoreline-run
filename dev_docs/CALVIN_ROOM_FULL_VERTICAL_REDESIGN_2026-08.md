...
13 docks (was 14). **Mandatory-rise audit (rev 1.1, after Lucas's playtest): worst 68 px (ground→A1)** — S06 was lowered to 55 px above M1's center in the placement-tightening revision, so the platform rise is again the worst mandatory move; everything ≤68 vs the 84.5 ceiling. No blind leaps — every target is visible before takeoff (M1/C1 both readable from H).

**Rev 1.1 (playtest revision):** (a) arc fragments biased to their landing/anchor platforms so each reads as *that dock's* sketch at rest, not a floater — S02→(630,382), S03→(1322,350), S07→(3335,392); (b) **S06→(2900,225)**, a correction the revision itself surfaced: pickups are body-overlap and the player is 62 px tall, so any fragment whose bottom sits below standing head-reach (M1 top − 62 = y 278) is collectable *without jumping* — both 268 and 285 silently were. At 225 the fragment is fully above standing reach: Earth Eyes makes a deliberate, retryable hop (mandatory, 8/8), and it sits on Red Bart's natural glide body-line. The audit metric for fragments is now head-reach, not platform-to-center; (c) Bart run-pose footing: measured seat gap was only 2 px — the airborne read is the *pose* (trailing-toe art). Code mitigations: 4 px foot sink plants the toes, and the contact shadow strengthens on Calvin night scenes (0.52 alpha, +15% size) where 0.34 vanished on dark planks. **The complete footing fix is a standing-pose Bart frame — an art request, family-gated, Lucas's call.**
